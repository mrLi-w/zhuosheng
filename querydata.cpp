...
Printer *QueryData::printer = NULL;

myThread::myThread(QObject *parent) ://读数据库线程
    QThread(parent)
{
}
...
void myThread::run()
{
   ...
   QString filename1 = QString("/dist/%1data.csv").arg(elementPath);

   QString tablename2 = "error";
   QString filename2 = QString("/dist/%1error.csv").arg(elementPath);

   QString tablename3 = "calibration";
   QString filename3 = QString("/dist/%1calibration.csv").arg(elementPath);

   QString tablename4 = "log";
   QString filename4 = QString("/dist/%1log.csv").arg(elementPath);

   QString tablename5 = "qc";
   QString filename5 = QString("/dist/%1qc.csv").arg(elementPath);
...
void myThread::readData(QString &table, QString &filename)
{
    ...
    QSqlTableModel *exportModel = new QSqlTableModel(NULL, sqldb);
    ...
    if (csvFile.open(QIODevice::ReadWrite))
    {
        ...
        stream << "No,";
        for(int i=1; i<exportModel->columnCount();i++)
        {
...
myThread2::myThread2(QObject *parent) ://查数据库线程
    QThread(parent)
{
    itemcount = 0;
    queryItemsEndID = 0;
...
void myThread2::run()
{
    ...
    QString strquery;
    strquery = QString("SELECT ID,%1 FROM %2").arg(myitems).arg(mytable);
    ...
    emit isDone(itemcount,queryItemsEndID);
}

void myThread2::getable(QStringList s1)
{
    mytable = s1.at(0);
    myitems = s1.at(1);
}
    ...
    row(urow)
{
    ...
    connect(this,SIGNAL(sendtable(QStringList)),thread2,SLOT(getable(QStringList)),Qt::QueuedConnection);
    connect(thread2,SIGNAL(isDone(int,int)),this,SLOT(dealDone2(int ,int)),Qt::QueuedConnection);
...
//    timer->start(10000);
//    connect(timer, SIGNAL(timeout()), this, SLOT(getQueryTotalItemCountAndEndId()));
...
void QueryData::InitModel()
{
    ...
    QFont font;
    font.setBold(true);
    font.setFamily("Sans Serif,9");

    //设置表格头部
    tvShowData->horizontalHeader()->setFont(font);
    tvShowData->setFont(QFont("Sans Serif", 8));
    tvShowData->horizontalHeader()->setFixedHeight(35);
    for(int i=0;i<column;i++){
...
int QueryData::runInthread(QString &table)//导出数据库表格文件.csv
{
    ...
    if(dsd.showModule())
    {
        ...
        {
            QString strSource1 = strTargetDir;
            QString strp = QString("cp /dist/%1%2.csv %3").arg(elementPath).arg(table).arg(strSource1);
//            qDebug() << strp;
            system(strp.toLatin1().data());
...
void QueryData::InitSlots()
{
    ...
    connect(endDate,     SIGNAL(dateTimeChanged(QDateTime)), this, SLOT(slot_QueryDateChange(QDateTime)));
    ...
    connect(pbPrint,  SIGNAL(clicked()), this, SLOT(slot_PrintData()));
}
...
void QueryData::slot_QueryDateChange(QDateTime)
{
    QSqlDatabase sqldb;
    if (!getUserDataBase(sqldb)){
        //lbPages->setText(tr("未连接数据库！"));
    ...
    QString timeID1 = endDate->dateTime().toString("yyyyMMddhhmm")+"99";
...
void QueryData::queryData(int page)
{
    ...
    totalPagesCnt = getTotalPages(itemcount, row);
    if( page < 1 )
        curpage = 1;
    ...
    currentPage->setText(tr("页码：%1/%2").arg(curpage).arg( totalPagesCnt ));
}
...
void QueryData::PrinterPageData()
{
    ...
    for(int i=0; i<=row; i++)
    {
        ...
        if(elementPath=="TP/"||elementPath=="TN/"||elementPath=="NH3N/")
        {
            ...
            element = "COD";
        QString st =printerPageData.at(0)+ " " + element + ":" +printerPageData.at(2)+ "mg/L"+"\n";
        printer->printValue(st);


        qDebug()<<st;
    }
}
void QueryData::printSelectRow()
{
   QString middata = "";
    printerData.clear();
    QModelIndex pageIndex;
#if !defined (Q_OS_WIN)
    QModelIndexList list=tvShowData->selectedIndexes();
#else
    QModelIndexList list;
#endif

    //int m = list.count();
    //int n = tvShowData->column;
    //list.count=m / n;
    qDebug()<<list.count();
    for (int i = 0; i < list.count()/column; i++)
    {
        printerPageData.clear();
        for(int j=0; j<3; j++)
        {
            pageIndex = model->index(list[i].row(),j);
            middata = model->data(pageIndex).toString();
            //qDebug()<<middata;
            printerPageData.append(middata);
        }
...
void QueryData::slot_PrinterData(QModelIndex index)
{
    ...
    qDebug()<<midStr;
//    QItemSelectionModel *selectModel = model->selectionModel();
//    QString copyStr;
...
void QueryData::slot_PrintData()//打印选中数据
{
    //QString str = time + " " + elementPath.left(elementPath.length()-1) + ":" + strResult + "mg/L";
    //QString str = "20-08-01 19:00"+" " +"COD化学需氧量:"+"20.123"+"mg/L";
    //printer->printValue("20-08-01 19:00","COD化学需氧量:","20.123","mg/L");
//    if(printSelect->currentIndex()==0)//打印当前页
//    {
//        PrinterPageData();
//    }
//    else if(printSelect->currentIndex()==1)//打印选中数据
//    {
        printSelectRow();
//    }
}
...
void QueryData::dealDone2(int a1, int a2)
{
    itemcount = a1;
    queryItemsEndID = a2;
    thread2->quit();
