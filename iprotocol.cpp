...
QString Sender::translateExplainCode()
{
    ...
    {
        i = profile.value("Loop2",0).toInt();
        j = profile.value("Loop3",0).toInt();
    }
    QString str;
    ...
    case 5:str = getTCValve1Name(TCValve1()) + QObject::tr("润洗");
            runxi++;
            if(runxi==1)
            {
                Time3 = QDateTime::currentDateTime().toString("yyyyMMddhhmmss");
                Time4 = QDateTime::currentDateTime().toString("yy-MM-dd hh:mm");
            }
            break;
    case 6:
        if(elementPath=="NH3N/"||elementPath=="TP/"||elementPath=="TN/")
        {
            str = QObject::tr("进") + getTCValve1Name(TCValve1());
        }
        else
        {
            if(TCValve1()==1)//纯水
            {
                ...
                if(m<j||m==j)
                {
                    str = QObject::tr("进") + getTCValve1Name(TCValve1())+ QString("%1/%2").arg(m).arg(j);
                }
                else
                {
                    str = QObject::tr("进") + getTCValve1Name(TCValve1());
            ...
            if(TCValve1()==3)//水样
            {
                if(n<i||n==i)
                {
                    str = QObject::tr("进") + getTCValve1Name(TCValve1())+ QString("%1/%2").arg(n).arg(i);
                }
                else
                {
                    str = QObject::tr("进") + getTCValve1Name(TCValve1());
    ...
    case 15:str = QObject::tr("水样排空");break;
    case 16:str = QObject::tr("空闲");break;
    }
    return str;
}
...
int Sender::TCValve1()
    {
      if(sent[12] == 'A')
            ...
            return sent.mid(12, 1).toInt();}
int Sender::TCValve2()
{
  if(sent[13] == 'A')
        return 10;
  else
        return sent.mid(13, 1).toInt();}
...
bool Sender::BlankReachStep(){return judgeStep() == 6;}
bool Sender::BlankJudgeStep(){return judgeStep() == -1;}

bool Sender::Washway()
{
    return judgeStep() == 7;
}
bool Sender::WashJudgeStep(){return judgeStep() == -1;}
...
void Sender::setwaterLedControl(int i)
{
   sent.replace(36, 1, QString("0000%1").arg(i).right(1).toLatin1());
}

void Sender::setmeasureLedControl(int i)
{
    sent.replace(37, 1, QString("0000%1").arg(i).right(1).toLatin1());
}

void Sender::settimeFix(int i)
{
    sent.replace(43, 2, QString("0000%1").arg(i).right(2).toLatin1());
}

void Sender::settimeAddFix(int i)
{
    sent.replace(45, 4, QString("0000%1").arg(i).right(4).toLatin1());
}

void Sender::settempFix(int i)
{
    sent.replace(49, 2, QString("0000%1").arg(i).right(2).toLatin1());
}

void Sender::setloopFix(int i)
{
    sent.replace(51, 2, QString("0000%1").arg(i).right(2).toLatin1());
}

void Sender::setjudgeStep(int i)
{
    sent.replace(53, 1, QString("0000%1").arg(i).right(1).toLatin1());
}

void Sender::setexplainCode(int i)
{
     sent.replace(55, 2, QString("0000%1").arg(i).right(2).toLatin1());//
}

void Sender::setcollectValue(int i)
{
    sent.replace(54, 1, QString("0000%1").arg(i).right(1).toLatin1());
}

//void Sender::settimeFix(int i){sent.replace(14, 1, QString("0000%1").arg(i).right(1).toLatin1());}
...
void ConfigSender::setWaterLevel4Threshold(int i)
{sent.replace(30, 4, QString("0000%1").arg(i).right(4).toLatin1());}
    ...
    counter(new ProtocolCounter(this))
{
    ...
    if (strlist.count() == 5)
    {
        ...
        if (port->isOpen()) {
            ...
            timer->stop();
            addErrorMsg(QString("上下位机通信端口打开失败%1  E18").arg(port->portName()), 1);
        }
    }
    else
