...
float smooth::calc(float rs)//平滑处理计算
{
    float result = rs;
    if(result > max2value || result == max2value )
    {
        if( rs3>0.0 )
        {
    ...
    else
    {
        if( rs3>0.0 )
        {
            if(fabs(rs3-rs)<dif2value||fabs(rs3-rs)==dif2value)
            {
                result = (rs2+rs3+rs)/3.0;
                rs1 = rs2;rs2 = rs3;rs3 = result;
            }
            ...
            if(fabs(rs2-rs)<dif2value||fabs(rs2-rs)==dif2value)
            {
                result = (rs2+rs1+rs)/3.0;
                rs3 = result;
            }
            ...
            if(fabs(rs1-rs)<dif2value||fabs(rs1-rs)==dif2value)
            {
                result = (rs1+rs)/2.0;
                rs2 = result;
            }
...
void smooth::setmax2value(float v)
{
    max2value = v;
}

void smooth::setdif2value(float v)
{
    dif2value = v;
}
