    ...
    files(f)
{
    ...
    fileSelect->setStyleSheet("font: 10pt Sans Serif;");
    for (int i = 0; i < files.count(); i++)
        fileSelect->addItem(files[i].fileName);

    insertBtn = new QPushButton(tr("插入"));
    insertBtn->setStyleSheet("font: 10pt Sans Serif;");
    removeBtn = new QPushButton(tr("移除"));
    removeBtn->setStyleSheet("font: 10pt Sans Serif;");
    copyBtn = new QPushButton(tr("复制"));
    copyBtn->setStyleSheet("font: 10pt Sans Serif;");
    pasteBtn = new QPushButton(tr("粘贴"));
    pasteBtn->setStyleSheet("font: 10pt Sans Serif;");
    updateBtn = new QPushButton(tr("更换命令"));
    updateBtn->setStyleSheet("font: 10pt Sans Serif;");
    okBtn = new QPushButton(tr("保存"));
    okBtn->setStyleSheet("font: 10pt Sans Serif;");
    cancelBtn = new QPushButton(tr("刷新"));
    cancelBtn->setStyleSheet("font: 10pt Sans Serif;");
...
void InstructionEditor::UpdateFile()
{
    ...
    if(dsd.showModule())
    {
        ...
        {
            system(QString("cp %1/*.txt ./%2").arg(strTargetDir).arg(elementPath).toLatin1().data());
