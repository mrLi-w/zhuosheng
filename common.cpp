...
QString Float4(float value)
{
    ...
    if(value < -99999.0)
        ...
        //qDebug()<<strRet;//.left(strRet>=0?5:6);
        if(strRet.endsWith("."))
            strRet = strRet.left(strRet>=0?4:5);
//        if(strRet.contains(".")&&strRet.endsWith("0"))
//        {
//            strRet.left(strRet.length()-1);
//        }
    }
    return strRet;
}
...
float setPrecision(float v, int n, QString *pshow)
{
    if (n >= 0)
    {
        float f = pow(10.0, n);
        if (pshow)
            *pshow =QString("%1").arg(v, 0, 'f', n); //QString::number(v,'f',n);//
...
void addMeasureData(QList<QVariant> &data)
{
    ...
    if (profile.beginSection("settings"))
       i = profile.value("MeasureTime",0).toInt();

    //QString TimeID;
    QString Time;
    QString Time7 = QDateTime::currentDateTime().toString("yyyyMMddhhmmss");
    QString Time8 = QDateTime::currentDateTime().toString("yy-MM-dd hh:mm");
    ...
    if (!sqlquery.exec(QString("INSERT INTO Data(ID,TimeID,A1,A2,A3,A4,A5,A6,A7,A8,A9,B1,B2,B3,B4,B5,B6,B7,B8,B9)"
                               "VALUES(NULL,'%1','%2'%3);")
                       .arg(Time).arg(TimeID).arg(strMsg)))
        qDebug() << sqlquery.lastError().text() << sqlquery.lastQuery();
    sqlquery.clear();

}

void addCheckData(QList<QVariant> &data)
{
    QSqlDatabase sqlitedb;
    if (!getUserDataBase(sqlitedb))
        return;
    ...
    QSqlQuery sqlquery(sqlitedb);
    if (!sqlquery.exec(QString("INSERT INTO QC(ID,TimeID,A1,A2,A3,A4,A5,A6,A7,A8,A9,B1,B2,B3,B4,B5,B6,B7,B8,B9)"
                               "VALUES(NULL,'%1','%2'%3);")
...
void addLogger(QString strMsg, LoggerType type)
{
    ...
    case LoggerTypeCheck: mesgType = QObject::tr("核查信息");break;
    case LoggerTypeSettingsChanged: mesgType = QObject::tr("参数变更");break;
    case LoggerLogin: mesgType = QObject::tr("登录信息");break;
    default:
...
int I420mAAdapter()
{
    ...
    if (profile.beginSection("settings")) {
        I4mA = profile.value("_4mA", 0.0).toFloat();
        I20mA = profile.value("_20mA", 1.0).toFloat();
    }

    if (profile.beginSection("measure"))
        cc = profile.value("conc").toFloat();

    if (cc > I20mA)
        cc = I20mA;

    float ii420 = I20mA - I4mA;
    if(ii420 <= 0)
        ii420 = 1;

    int val = (1600/ii420)*(cc-I4mA)+400;

    if((val >= 400) && (val<1000))
    {
        float tempdata = ((float)val - v420mA.b1)/ v420mA.k1;
        val = (int)tempdata;
        if(tempdata >= (float)val + 0.5)
            val++;
    }
    else if((val >= 1000) && (val<1500))
    {
        float tempdata = ((float)val - v420mA.b2)/ v420mA.k2;
        val = (int)tempdata;
        if(tempdata >= (float)val + 0.5)
            val++;
    }
    ...
    {
        float tempdata = ((float)val - v420mA.b3)/ v420mA.k3;
        val = (int)tempdata;
        if(tempdata >= (float)val + 0.5)
            val++;
    }
...
int ConcOutOfRange()
{
    ...
    if (profile.beginSection("settings")) {
        l = profile.value("AlarmLineL", 0.0).toFloat();
        h = profile.value("AlarmLineH", 1.0).toFloat();
    }
    return (cc < l || cc > h);
}
